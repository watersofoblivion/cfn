...
let test_pp_binding ctxt =
  ...
  fresh_binding ~patt ~ty ~value ()
    |> assert_pp_binding ~ctxt [
         fprintf str_formatter "%a: %a =@ @[<hv>%a@]" Clos.pp_patt patt Clos.pp_ty ty Clos.pp_expr value
           |> flush_str_formatter
       ]
...
let test_pp_term_let ctxt =
  ...
  fresh_term_let ~binding ~scope ()
    |> assert_pp_term ~ctxt [
         fprintf str_formatter "@[<hv>@[<b 2>let %a@]@ @]in@ %a" Clos.pp_binding binding Clos.pp_term scope
           |> flush_str_formatter
       ]
