...
let fresh_binding ?patt:(patt = PattTest.fresh_patt_ground ()) ?ty:(ty = Ir.ty_bool) ?value:(value = ExprTest.fresh_expr_atom ()) _ =
...
let test_pp_binding ctxt =
  ...
  fresh_binding ~patt ~ty ~value ()
    |> assert_pp_binding ~ctxt [
         fprintf str_formatter "%a: %a =@ @[<hv>%a@]" Ir.pp_patt patt Ir.pp_ty ty Ir.pp_expr value
           |> flush_str_formatter
       ]
...
let test_pp_term_let ctxt =
  ...
  fresh_term_let ~binding ~scope ()
    |> assert_pp_term ~ctxt [
         fprintf str_formatter "@[<hv>@[<b 2>let %a@]@ @]in@ %a" Ir.pp_binding binding Ir.pp_term scope
           |> flush_str_formatter
       ]
